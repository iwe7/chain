import { Type, ObjectLiteral } from "@imeepos/core";
import { keys, Table } from "@imeepos/util";
...
export function getMetadata<T>(target: Type<any>): MetadataDef<T>[] {
  let res = [];
  metadataTable.data.forEach((data, k) => {
    if (data.target === target) {
      res.push({ __id__: k, ...data });
    }
  });
  return res;
}
